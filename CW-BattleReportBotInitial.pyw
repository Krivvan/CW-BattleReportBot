...
def loadJSON():
  ...
  currentURL = "https://static.mwomercs.com/data/cw/mapdata-" + year + "-" + month + "-" + day + "T06-15" + ".json"
  jsonurl = urllib.urlopen(currentURL)
  currentData = json.loads(jsonurl.read())

  pastURL = "https://static.mwomercs.com/data/cw/mapdata-" + year + "-" + month + "-" + day + "T06-00" + ".json"
  jsonurl = urllib.urlopen(pastURL)
  pastData = json.loads(jsonurl.read())

  date = "**" + "NA - 06:00UTC - " + month + "/" + day + "/" + year + "**  \n\n"
  ...
  for id in range (1,2241):
    if (pastData[str(id)]["contested"] == "1"):
      if (pastData[str(id)]["owner"]["id"] == currentData[str(id)]["owner"]["id"]):
        ...
        if (id != 1):
          netChange[pastData[str(id)]["invading"]["id"]] = netChange[pastData[str(id)]["invading"]["id"]] + 1
...
def makeRedditPost():
  ...
  user.set_oauth_app_info(client_id='NotRealClientID', client_secret='NotRealClientSecret', redirect_uri='http://127.0.0.1:65010/authorize_callback')
  ...
  title = "DAY " + num2words(day).upper() + " BATTLE REPORT"
  
  submissions = user.get_subreddit(SUBREDDIT).get_new(limit=100)
  repostFound = False
  #for submission in submissions:
  #  if (title == (submission.title.encode('ascii')) and USERNAME == submission.author.name.encode('ascii')):
  #    repostFound = True

  if (not repostFound):
    body = createPostContent()
    newestSaved = authenticated_user.get_saved('new').next()
    body = body + "[**Link to Day " + str(day-1) + " Battle Report here.**](" + newestSaved.url.encode('ascii') + ")\n\n"
    body = body + "^^Message ^^/u/Krivvan ^^if ^^something ^^is ^^wrong ^^with ^^this ^^automatically ^^generated ^^report.\n\n"
    
    #print body
    
    submission   = user.submit(SUBREDDIT, title, body, save = True, resubmit = False)
    submission.save()
    authenticated_user.select_flair(submission, u'b566b4e0-8321-11e4-9a61-22000bc0c4dc')
